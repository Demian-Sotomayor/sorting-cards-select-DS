...
btnSort.addEventListener("click", e => {
  ...
  for (let i = 0; i < initialOrder.length - 1; i++) {
    ...
    for (let b = i + 1; b < initialOrder.length; b++) {
      if (initialOrder[b].number < initialOrder[minIndex].number) {
        minIndex = b;
      }
    }
    ...
    if (minIndex !== i) {
      ...
      initialOrder[i] = initialOrder[minIndex];
      initialOrder[minIndex] = temp;
    }
